...
//        insertData("Leo" + time);
    ...
    public void queryData(String tempName) {
        ...
        Cursor cursor = helper.getReadableDatabase().rawQuery(
                "select id as _id,name from records where name like '%" + tempName + "%' order by id desc limit 0,5 ", null);
        // 创建adapter适配器对象,装入模糊搜索的结果
        adapter = new SimpleCursorAdapter(context, R.layout.city_data, cursor,new String[] { "name" },
                new int[] { R.id.text_id }, CursorAdapter.FLAG_REGISTER_CONTENT_OBSERVER);
        // 设置适配器
